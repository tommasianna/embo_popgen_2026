...
You may find this material helpful:
...
- The R script
  [ABC_demo.r](https://bitbucket.org/wegmannlab/statistical_modelling_and_inference/raw/master/Scripts/ABC_demo.r)
  used to illustrate ABC.

<!-- %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% -->

## Exercises A: Getting started with ABC

<!-- %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% -->

### Step A1: Compile the latest version of ABCtoolbox and check executables

I suggest you work in the directory `Daniel_Wegmann/day_5/`. Once in
that directory, let us begin by creating a directory “bin” for all
executables (binaries) of programs you will use. Then enter that
directory.
...
Now lets compile ABCtoolbox! With a reasonably recent version of gcc
available, the compilation is done easily by changing into the directory
of the code and using the provided make file by simply typing
...
former (you may also copy the version you used yesterday):

``` bash
wget http://cmpg.unibe.ch/software/fastsimcoal28/downloads/fsc28_linux64.zip
...
folder to your PATH (you will have to redo this if you reconnect).
...
constant population size. You find all necessary files in the folder
`ABCDemo`. Change into that directory.
...
will. In that directory you will find a file with the ending `.arp`,
...
1.  the file `arl_run.ars` that tells arlsumstats what statistics are to
    be calculated
...
possible. Have a look at those file, but note that both files can also
be generated and modified with the graphical version of Arlequin.
...
We will now use ABC to estimate the parameters we used to generate this
data.
...
figure out how the use of external programs is specified and esnure the
paths are correct. Note that while we here use `fastsimcoal2`and
`arlsumstat`, `ABCtoolbox` would work with any other program too. Note
also that `fastimcoal2` is to be run with an input file on its own. Here
we will use the file `constsize.par`, which contains tags for the
parameters that are to be sampled from the prior distributions.

**Q1)** Also, have a look at how the prior are specified. Why are we not
using the logunif prior distribution provided by `ABCtoolbox` directly
on `N_NOW`?
...
Both would work, but the difference is whether we will estimate
`LOG10_N_NOW` or `N_NOW`. The former is preferred because i) the
simulations are spread uniformy over the log range and ii) because the
log of the population size is often correlating better with the summary
statistics. For both reasons, the regression-based estimation is usually
more accurate when working with the log of population sizes, at least if
the prior range spans more than one order of mangnitude.
</details>
...
ABCtoolbox will complain that there are highly correlated statistics and
remove two of the statistics (read the log carefully to spot that info).
Once the estimation has finished successfully, the output was written to
a series of files with tag `ABC_estimation_constsize_`, as was specified
in the input file.
...
yourself so that you get a feel for the structure of the output.

Begin by plotting the marginal posterior distributions of the population
size (`N_NOW`) and the mutation rate (`MUTATION_RATE`) as follows (make
sure your session uses `ABCDemo` as working directory or adjust the code
accordingly):
...
Do you notice something? Very “surprisingly”, we can not estimate N and
mu together! So let’s try to estimate *θ* = 2*N**μ* instead. (wonder why
...
Have a look at it. Note that the same par file can be used!

**Q2)** Generate an input file `constsize_theta.input` for ABCtoolbox to
...
`sims_constsize_theta_`.
<details>
<summary>
Solutions
</summary>

You should have adjusted `estName`, `outName` as follows:
    ...
    estName constsize_theta.est
    outName sims_constsize_theta

    obsName constsize.obs
    simDataName constsize-temp_1_1.arp
    numSims 10
    simProgram ../bin/fastsimcoal2
    simInputName constsize.par
    simArgs -i SIMINPUTNAME -n 1 -q
    sumStatProgram ../bin/arlsumstat
...
Then, you can generate simulations as follows:
...
**Q3)** Now estimate *θ* using `ABCtoolbox`. For this, create an input
file `estimate_theta.input` by copying and then modifying the input file
we used for the previous model. Also, change the output prefix to
...
You should have changes `simName` and `outputPrefix` and removed the
request for joint posteriors as we no only have one parameter to
estimate.

    //----------------------------------------------------------------------
    //ABCtoolbox input file to estimate theta
    //----------------------------------------------------------------------
    task estimate
    ...
    //simulated data
    simName sims_constsize_theta_sampling1.txt
    params 2 
    outputPrefix ABC_estimation_constsize_theta_
...
**Q4)** You can now plot the marginal posterior for *θ* in R. How well
does the estimate fit the parameters we used to generate the observed
data? Check the file `dna_singlepop_constsize_obs.par` to see what
parameters we used and remember that *θ* = 2*N**μ* (for haploid
species).
...
You can plot the posterior like this:

``` r
post <- read.table("ABC_estimation_constsize_theta_model0_MarginalPosteriorDensities_Obs0.txt", header=T)
plot(post$LOG10_THETA, post$LOG10_THETA.density, type='l')
abline(v=log10(10000 * 10^-7), col = 'purple', lty = 2)
```

You should now get a rather narrow peak that contains the true value you
used.
</details>

<br/>

**Q5)** Change the parameters in `constsize_obs.par` and regenerate the
observed data, re-calculate summary statistics from them and rerun the
estimation (no need to generate new simulations!). Can you accurately
estimate *θ*?

<!-- %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% -->

## Exercises B (extra if time permits of for after the course): Choice of Summary Statistics & Validation
...
install it. To do so, open R on the command line (using the command `R`)
and then type
...
## Exercises C (extra if time permits of for after the course): Model Choice
...
1.  the current theta log10(THETA_CUR) ~ U\[-4, -1\],
2.  the old population size before the bottleneck relative to the
    current size log10(OLD_SIZE_RELATIVE) ~ U\[-2,2\] and
3.  the time when the bottleneck occurred log10(T_BOT) ~ U\[1,3\].
