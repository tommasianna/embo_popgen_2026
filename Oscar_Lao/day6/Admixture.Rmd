...
{
  # African ancestral population
  popA <- population("popA", time = 2*time_split_A_B, N = Ne1)
  popB <- population("popB", parent = popA, time = time_split_A_B, N = Ne2)
  model_nogf <- compile_model(populations = list(popA, popB), generation_time = 1,  simulation_length = 2*time_split_A_B)
  samples <- schedule_sampling(
  model_nogf, times = 0,
  list(popA, sample1), list(popB, sample2)
)
  return(model_nogf);
}
...
#########################################################################
#
# 1.2. What happens if the effective population size is different between populations? What if the sample size is different? Change the previous code so pop1 has an effective population size 10 times larger than pop2
#
#
...
basic_model <- function(time_split_A_B, NeA, NeB, sampleA, sampleB)
{
  # African ancestral population
  popA <- population("popA", time = 2*time_split_A_B, N = NeA)
  popB <- population("popB", parent = popA, time = time_split_A_B, N = NeB)
  model_nogf <- compile_model(populations = list(popA, popB), generation_time = 1,  simulation_length = 2*time_split_A_B)
  samples <- schedule_sampling(
  model_nogf, times = 0,
  list(popA, sampleA), list(popB, sampleB)
)
  return(model_nogf);
}
...
model <- basic_model(time_split_A_B, NeA, NeB, sampleA, sampleB);
...
df <- as.data.frame(pca$x[,1:2])
colnames(df) <- c("PC1", "PC2")
df$population <- c(rep("popA", sampleA),rep("popB", sampleB))
...
# same demographic parameters, different sample size
time_split_A_B <- 10000;
NeA <- 1000;
NeB <- 1000;
sampleA <- 4;
...
Why are we observing this shape? What does that mean? Change the time of split to older and closer to present times. What do you see?
...
FasSimcoal2 is another coalescent-based simulator. It was originally developed by Laurent Excoffier. This software is interesting because
...
#########################################################################
# 2.1. Project in two dimensions the relationships of a dataset generated with the model.admixed.pop generated with fastSimcoal2 (another simulator)
