...
# 1. Read pairwise Fst files

fst_afr_eas <- read.table("input/Part_1_HumanDiversity/AFR_EAS.weir.fst", header=TRUE)

fst_afr_eur <- read.table("input/Part_1_HumanDiversity/AFR_EUR.weir.fst", header = TRUE)

fst_eas_eur <- read.table("input/Part_1_HumanDiversity/EAS_EUR.weir.fst", header = TRUE)


# 2. Filter duplicate SNP positions and exclude NA values

no_dup_fst_afr_eas <- fst_afr_eas[!duplicated(fst_afr_eas$WEIR_AND_COCKERHAM_FST), ]

no_dup_fst_afr_eur <- fst_afr_eur[!duplicated(fst_afr_eur$WEIR_AND_COCKERHAM_FST), ]

no_dup_fst_eas_eur <- fst_eas_eur[!duplicated(fst_eas_eur$WEIR_AND_COCKERHAM_FST), ]


filt_fst_afr_eas <- no_dup_fst_afr_eas[!is.na(no_dup_fst_afr_eas$WEIR_AND_COCKERHAM_FST), ]

filt_fst_afr_eur <- no_dup_fst_afr_eur[!is.na(no_dup_fst_afr_eur$WEIR_AND_COCKERHAM_FST), ]

filt_fst_eas_eur <- no_dup_fst_eas_eur[!is.na(no_dup_fst_eas_eur$WEIR_AND_COCKERHAM_FST), ]


# 3. Align the datasets by overlapping positions.

# Create SNP IDs
filt_fst_afr_eas$SNP <- paste(filt_fst_afr_eas$CHROM,
                              filt_fst_afr_eas$POS,
                              sep = "_")

filt_fst_afr_eur$SNP <- paste(filt_fst_afr_eur$CHROM,
                              filt_fst_afr_eur$POS,
                              sep = "_")

filt_fst_eas_eur$SNP <- paste(filt_fst_eas_eur$CHROM,
                              filt_fst_eas_eur$POS,
                              sep = "_")
...
common_snps <- Reduce(
  intersect,
  list(
    filt_fst_afr_eas$SNP,
    filt_fst_afr_eur$SNP,
    filt_fst_eas_eur$SNP
  )
)
...
afr_eas <- fst_afr_eas %>% filter(SNP %in% common_snps)
afr_eur <- fst_afr_eur %>% filter(SNP %in% common_snps)
eas_eur <- fst_eas_eur %>% filter(SNP %in% common_snps)

# Order
afr_eas <- afr_eas %>% arrange(SNP)
afr_eur <- afr_eur %>% arrange(SNP)
eas_eur <- eas_eur %>% arrange(SNP)

# 4. Set negative Fst to 0
afr_eas$WEIR_AND_COCKERHAM_FST <- pmax(afr_eas$WEIR_AND_COCKERHAM_FST, 0)
afr_eur$WEIR_AND_COCKERHAM_FST <- pmax(afr_eur$WEIR_AND_COCKERHAM_FST, 0)
eas_eur$WEIR_AND_COCKERHAM_FST <- pmax(eas_eur$WEIR_AND_COCKERHAM_FST, 0)
...
rehh_format_EAS <- data2haplohh(hap_file = "Chr2_EDAR_CHS_500K.recode.vcf", polarize_vcf = FALSE, vcf_reader = "vcfR")

rehh_format_AFR <- data2haplohh(hap_file = "Chr2_EDAR_LWK_500K.recode.vcf", polarize_vcf = FALSE, vcf_reader = "vcfR")                 


# 2 Estimate the EHH decay for rs3827760 in both populations.

ehh_decay_EAS <- calc_ehh(haplohh = rehh_format_EAS, 
                          mrk = "rs3827760", 
                          include_nhaplo = TRUE)

ehh_decay_AFR <- calc_ehh(haplohh = rehh_format_AFR, 
                          mrk = "rs3827760", 
                          include_nhaplo = TRUE)                     
...
plot(ehh_decay_EAS,
     main = "EHH decay - EAS (EDAR)",
     xlab = "Distance",
     ylab = "EHH",
     col = "red")

plot(ehh_decay_AFR,
     main = "EHH decay - AFR (EDAR)",
     xlab = "Distance",
     ylab = "EHH",
     col = "blue")
...
ehh_scan_eas <- scan_hh(haplohh = rehh_format_EAS)

ehh_scan_afr <- scan_hh(haplohh = rehh_format_AFR)


# 2. Calculate iHS scores for both populations using `ihh2ihs()`.

iHS_eas <- ihh2ihs(ehh_scan_eas, min_maf = 0.02)

iHS_afr <- ihh2ihs(ehh_scan_afr, min_maf = 0.02)
...
**Write your R code here:**
  ```R
# Read the pairwise $F_{ST}$ files involving Native Americans (`input/Part_1_HumanDiversity/Chr2_NAM_EAS.weir.fst` and `input/Part_1_HumanDiversity/Chr2_NAM_EUR.weir.fst`) and Europeans-East Asians (`input/Part_1_HumanDiversity/Chr2_EUR_EAS.weir.fst`).

fst_nam_eas <- read.table("Chr2_NAM_EAS.weir.fst", header=TRUE)

fst_nam_eur <- read.table("Chr2_NAM_EUR.weir.fst", header = TRUE)

fst_eas_eur <- read.table("Chr2_EUR_EAS.weir.fst", header = TRUE)



# 2. Filter duplicate SNP positions and exclude NA values

no_dup_nam_eas <- fst_nam_eas[!duplicated(fst_nam_eas$WEIR_AND_COCKERHAM_FST), ]

no_dup_nam_eur <- fst_nam_eur[!duplicated(fst_nam_eur$WEIR_AND_COCKERHAM_FST), ]

no_dup_eas_eur <- fst_eas_eur[!duplicated(fst_eas_eur$WEIR_AND_COCKERHAM_FST), ]


filt_nam_eas <- no_dup_nam_eas[!is.na(no_dup_nam_eas$WEIR_AND_COCKERHAM_FST), ]

filt_nam_eur <- no_dup_nam_eur[!is.na(no_dup_nam_eur$WEIR_AND_COCKERHAM_FST), ]

filt_eas_eur <- no_dup_eas_eur[!is.na(no_dup_eas_eur$WEIR_AND_COCKERHAM_FST), ]


# 3. Align the datasets by overlapping positions.

# Create SNP IDs
filt_nam_eas$SNP <- paste(filt_nam_eas$CHROM,
                          filt_nam_eas$POS,
                              sep = "_")

filt_nam_eur$SNP <- paste(filt_nam_eur$CHROM,
                          filt_nam_eur$POS,
                              sep = "_")

filt_eas_eur$SNP <- paste(filt_eas_eur$CHROM,
                          filt_eas_eur$POS,
                              sep = "_")
...
# PCA da PLINK
pca <- read.table("plink_pca.eigenvec", header = FALSE)

# Eigenvalues
eigval <- scan("plink_pca.eigenval")

# Sample info (metadata)
meta <- read.table("sample_info.txt",
                   header = TRUE, sep = "\t")

colnames(pca)[1:2] <- c("FID", "IID")

# PC columns (da PLINK partono dalla 3 in poi)
pc_names <- paste0("PC", 1:(ncol(pca) - 2))
...
data <- pca %>%
  left_join(meta, by = c("IID" = "sampleName"))
...
ggplot(data, aes(x = PC1, y = PC2, color = breed, shape = group)) +
  geom_point(size = 3) +
  xlab(paste0("PC1 (", round(pc1, 2), "%)")) +
  ylab(paste0("PC2 (", round(pc2, 2), "%)")) +
  theme_minimal()
...
filename <- "subset_chr15"

pcadapt_data <- read.pcadapt(filename, type = "bed")
